# Operator variables
# ==================
export APP_NAME=selinux-policy-installer-operator
...
.PHONY: all
all: build ## Test and Build the selinux-policy-installer-operator
...
.PHONY: build
build: ## Build the selinux-policy-installer-operator binary
	$(GO) build -o $(TARGET) github.com/JAORMX/selinux-policy-installer-operator/cmd/manager
...
.PHONY: run
run: operator-sdk ## Run the selinux-policy-installer-operator locally
	WATCH_NAMESPACE=$(NAMESPACE) 
	KUBERNETES_CONFIG=$(KUBECONFIG) \
	OPERATOR_NAME=selinux-policy-installer-operator \
...
# probably want to push the selinux-policy-installer-operator image to the cluster we're
...
images-to-cluster: namespace openshift-user image
