...
import (
	...
	"github.com/JAORMX/selinux-policy-installer-operator/pkg/controller/utils"
)
...
func (r *ReconcileConfigMap) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if !ok {
		reqLogger.Info("Found ConfigMap with no 'appName' label. Skipping", "ConfigMap.Namespace", cminstance.Name)
		return reconcile.Result{}, nil
	}
	policyNamespace, ok := cminstance.Labels["appNamespace"]
	if !ok {
		reqLogger.Info("Found ConfigMap with no 'appNamespace' label. Skipping", "ConfigMap.Namespace", cminstance.Name)
		return reconcile.Result{}, nil
	}
	...
	return &corev1.Pod{
		...
		Spec: corev1.PodSpec{
			Containers: []corev1.Container{
				corev1.Container{
					Name:    utils.GetInstallerPodName(name, ns, node),
					Image:   "image-registry.openshift-image-registry.svc:5000/selinux-operator/udica:latest",
